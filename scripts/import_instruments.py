...
class SpiderTool:
    ...
    def handle_errors(self, error: server_api.ServerError):
        ...
        if (error.status_code == 409):
            logging.error(f"Error {error.status_code} inserting {error.data['name']}: {error.message}. Duplicate (multiple) instruments fetched from the server.")
        
        # Handling Server Errors
        if (error.status_code >= 500):
            logging.error(f"Server Error {error.status_code} inserting {error.data['name']}: {error.message}.")

        # Anything else than NOT FOUND, is an error
        if (error.status_code != 404):
            logging.error(f"Error {error.status_code} inserting {error.data['name']}: {error.message}.")
    ...
    def update(self, data: dict) -> dict:
        ...
        response = self.api.update_instrument(data['doi'], data)

        logging.info(f"Sucessfully updated {response['name']}")
        return response
    ...
    def process_instrument(self, data: dict):
        try:
            ...
            if not existing:
                ...
                updated = self.comparator.compare_instruments(data, existing)
            
            # handling if there is an update
            if (updated != existing):
                existing = self.update(updated) # posted (PUT) the updates in the server
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--test', action='store_true', help='Test mode, only get data from website, do not send updates')
